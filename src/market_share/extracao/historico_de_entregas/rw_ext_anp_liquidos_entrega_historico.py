...
def rw_ext_anp_liquidos_entrega_historico():
    """
    Faz download do arquivo mais recente de Market Share "Líquidos Entregas Histórico" do bucket no GCP,
    ...
    blobs_filtrados = [b for b in blobs if "LIQUIDOS_ENTREGAS_HISTORICO" in b.name]
    if not blobs_filtrados:
        raise FileNotFoundError("Nenhum arquivo encontrado com 'LIQUIDOS_ENTREGAS_HISTORICO' no nome.")
    ...
    print(df[['regiao_origem', 'regiao_destinatario']].head())
...
if __name__ == "__main__":
	rw_ext_anp_liquidos_entrega_historico()
